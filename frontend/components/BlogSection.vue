            ...
            <div v-else class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                <article v-for="post in posts" :key="post.id" class="flex flex-col items-start">
                    <div class="relative w-full">
                        <img :src="post.attributes.image.url"  :alt="post.attributes.image.data.attributes.alternativeText"
                             class="aspect-[16/9] w-full rounded-2xl bg-gray-100 object-cover sm:aspect-[2/1] lg:aspect-[3/2]"/>
                        <div class="absolute inset-0 rounded-2xl ring-1 ring-inset ring-gray-900/10"/>
                        <p class="absolute bottom-2 left-2 text-xs z-10 rounded-full bg-[#080321] px-2 py-1 text-gray-100">Category</p>
                    </div>
                    <div class="max-w-xl">
                        <div class="mt-8 flex items-center gap-x-4 text-xs">
                            <time class="text-gray-100">{{ post.attributes.createdAt }}</time>
                            <span class="text-gray-100">|</span>
                            <p class="font-semibold text-gray-100">{{ post.attributes.author }}</p>
                        </div>
                        ...
                        <div class="mt-8 flex items-center gap-x-6">
                            <nuxt-link :to="`/posts/${post.id}`" class="text-sm font-semibold text-gray-100">
